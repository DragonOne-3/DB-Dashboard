...
def digits(v): return ''.join(ch for ch in str(v or '') if ch.isdigit())
...
def date_key(row):
    ...
    for k,v in row.items():
        if any(x in k for x in ('일자','날짜','Date','Dt')):
            d=digits(v)
            if len(d)>=8:return d[:8]
    return ''
...
def category(title):
    ...
    if any(k.replace(' ','') in compact for k in CORE):
        if any(k in title for k in RENTAL): return '장비임차'
        return '장비구매'
...
def is_equipment(cat): return cat in {'장비구매','장비임차','부속·비품','유지보수'}

def status_of(row): return first(row,STATUS_COLUMNS) or '미상'
...
def dedupe(row,source):
    ...
    for c in IDENTIFIER_COLUMNS:
        if row.get(c) not in (None,''):ids.append(str(row[c]).strip())
...
def scan(client,name,source):
    ...
    for ws in ss.worksheets():
        ...
        for vr in vals[1:]:
            ...
            row.update({'_source':source,'_title':t,'_category':cat,'_amount':amount_of(row,source),'_date':date_key(row),'_deadline':deadline_key(row),'_agency':first(row,AGENCY_COLUMNS),'_company':first(row,COMPANY_COLUMNS),'_status':st,'_open':is_open(source,row,st),'_bid_rate':bid_rate(row),'_tab':ws.title})
            rows.append(row)
    return headers,rows

def monthly(rows):
    d=defaultdict(lambda:{'count':0,'amount':0,'open_count':0,'open_amount':0,'equipment_amount':0,'service_amount':0})
    for r in rows:
        m=r['_date'][:6]
        if len(m)!=6:continue
        b=d[m];b['count']+=1;b['amount']+=r['_amount']
        if r['_open']:b['open_count']+=1;b['open_amount']+=r['_amount']
        if is_equipment(r['_category']):b['equipment_amount']+=r['_amount']
        else:b['service_amount']+=r['_amount']
...
def agency_rank(rows):
    ...
    return sorted(out,key=lambda x:(x['open_amount'],x['amount'],x['count']),reverse=True)[:15]

def competitors(rows):
    a=defaultdict(lambda:{'count':0,'amount':0,'rates':[],'equipment_count':0,'service_count':0})
    for r in rows:
        ...
        if is_equipment(r['_category']):b['equipment_count']+=1
        else:b['service_count']+=1
    out=[]
    for k,v in a.items():
        out.append({'company':k,'count':v['count'],'amount':v['amount'],'avg_rate':round(sum(v['rates'])/len(v['rates']),2) if v['rates'] else None,'major':'장비' if v['equipment_count']>=v['service_count'] else '처리용역'})
    return sorted(out,key=lambda x:(x['amount'],x['count']),reverse=True)[:20]

def compact(r):
    return {'source':r['_source'],'title':r['_title'],'agency':r['_agency'],'company':r['_company'],'amount':r['_amount'],'status':r['_status'],'date':r['_date'],'deadline':r['_deadline'],'category':r['_category']}
...
def main():
    ...
    open_rows=sorted([r for r in all_rows if r['_open']],key=lambda r:(r['_amount'],r['_deadline'],r['_date']),reverse=True)
    ...
    payload={'updated_at':now.strftime('%Y-%m-%d %H:%M:%S'),'keyword':KEYWORD,'pipeline':pipeline,'monthly':month_data,'agencies':agencies,'competitors':{'계약정보':comp},'priority_leads':[compact(r) for r in open_rows[:20]],'kpi':{
    ...
    print('완료:',len(all_rows),'건, 진행중',len(open_rows),'건, 경쟁사',len(comp),'개사')
