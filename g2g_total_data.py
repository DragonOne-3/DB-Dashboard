...
st.markdown("""
    ...
    /* 버튼 텍스트 크기 축소 및 간격 조정 */
    div.stButton > button { width: 100%; height: 30px; font-size: 12px !important; padding: 0px !important; }
    </style>
...
def show_result_table(cat, idx_list):
    ...
    with ctrl_l: st.markdown(f"**✅ 조회결과: {len(df):,}건** (칼럼 클릭 시 전체 데이터가 정렬됩니다)")
    ...
    # st.dataframe의 on_select나 이벤트를 대신하여 기본 정렬 기능을 활용하되,
    # 사용자가 정렬을 시도하면 프레임워크가 세션 내의 df를 기준으로 다시 자르도록 유도
    edited_df = st.dataframe(
        df[show_cols].iloc[(curr_p-1)*p_limit : curr_p*p_limit], 
        use_container_width=True, 
        height=520,
        key=f"df_view_{cat}" # 키를 지정하여 상태 유지
    )
...
for i, tab in enumerate(tabs):
    ...
    with tab:
        ...
        _, center_area, _ = st.columns([0.5, 9, 0.5]) # 중앙 여백 조정
        with center_area:
            ...
            with r2_r:
                d1, d2, d3, d4 = st.columns([1.5, 1.5, 5.5, 1.5])
                
                # 날짜 입력 (세션 상태와 연동)
                sd_in = d1.date_input("시작", value=st.session_state[f"sd_{cat}"], key=f"sd_w_{cat}", label_visibility="collapsed")
                ed_in = d2.date_input("종료", value=st.session_state[f"ed_{cat}"], key=f"ed_w_{cat}", label_visibility="collapsed")
                st.session_state[f"sd_{cat}"] = sd_in
                st.session_state[f"ed_{cat}"] = ed_in

                # [퀵 버튼] - 종료일 기준 자동 계산
                q_cols = d3.columns(6)
                def set_quick(m=0, y=0):
                ...
                if q_cols[0].button("1개월", key=f"m1_{cat}"): set_quick(m=1)
                if q_cols[1].button("3개월", key=f"m3_{cat}"): set_quick(m=3)
                if q_cols[2].button("6개월", key=f"m6_{cat}"): set_quick(m=6)
                if q_cols[3].button("9개월", key=f"m9_{cat}"): set_quick(m=9)
                if q_cols[4].button("1년", key=f"y1_{cat}"): set_quick(y=1)
                if q_cols[5].button("2년", key=f"y2_{cat}"): set_quick(y=2)
        ...
        if search_exe:
            with st.spinner("데이터 분석 중..."):
                ...
                if not df_raw.empty:
                    ...
                    # [중요] 검색 시 기본 정렬은 날짜 최신순
                    df_filtered = df_filtered.sort_values(by='tmp_dt', ascending=False)
