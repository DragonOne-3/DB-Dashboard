...
st.markdown("""
    ...
    /* 2. 검색조건 위 네모칸(빈 컨테이너) 제거 */
    div[data-testid="stVerticalBlock"] > div:has(div.search-container) { display: none !important; }
    ...
    /* 4. 퀵버튼 수평 정렬 및 사이즈 (위로 끌어올림) */
    .q-btn-container { 
        margin-top: -12px !important; /* 입력창과 높이를 맞추기 위한 핵심 코드 */
    }
    .q-btn-container button { 
    ...
    /* 5. 결과 상단바 투명화 및 하단 페이지 버튼 */
    .data-info-bar { background-color: transparent !important; border-top: 1px solid #ddd; padding: 10px 0px; }
...
def show_result_table(cat, idx_list):
    """결과 데이터를 보여주고 정렬 및 다운로드를 수행합니다."""
    df = st.session_state.get(f"df_{cat}")
    if df is None: return

    # [중요] 정렬 로직: 드롭박스 방식으로 가장 안전하게 구현
    st.markdown('<div class="data-info-bar">', unsafe_allow_html=True)
    res_col, sort_col1, sort_col2, limit_col, dl_col = st.columns([1.5, 2, 2, 1.2, 3.3])
    ...
    # 데이터프레임을 직접 정렬 (TypeError 방지를 위해 복사본 사용)
    df_sorted = df.sort_values(by=sort_key, ascending=ascending).copy()
    ...
    with dl_col:
        ...
        d_csv.download_button("📑 CSV", df_sorted.to_csv(index=False, encoding='utf-8-sig').encode('utf-8-sig'), f"{cat}.csv", "text/csv")
    ...
    # 테이블 및 페이지네이션
    total_pages = max((len(df_sorted) - 1) // p_limit + 1, 1)
    curr_p = st.session_state.get(f"p_num_{cat}", 1)
    if curr_p > total_pages: curr_p = total_pages
    st.dataframe(df_sorted[show_cols].iloc[(curr_p-1)*p_limit : curr_p*p_limit], use_container_width=True, height=520)
...
for i, tab in enumerate(tabs):
    ...
    with tab:
        # [핵심 에러 해결] 세션 날짜를 반드시 파이썬 순수 date 객체로 관리
        if f"sd_{cat}" not in st.session_state: st.session_state[f"sd_{cat}"] = date.today() - relativedelta(months=6)
        if f"ed_{cat}" not in st.session_state: st.session_state[f"ed_{cat}"] = date.today()
        ...
        with center_area:
            ...
            with r2_r:
                ...
                sd_in = d1.date_input("시작", value=s_val, key=f"sd_w_{cat}_{st.session_state[f'ver_{cat}']}", label_visibility="collapsed")
                ed_in = d2.date_input("종료", value=e_val, key=f"ed_w_{cat}_{st.session_state[f'ver_{cat}']}", label_visibility="collapsed")
                st.session_state[f"sd_{cat}"], st.session_state[f"ed_{cat}"] = sd_in, ed_in

                # 퀵버튼 (CSS q-btn-container 적용)
                with d3:
