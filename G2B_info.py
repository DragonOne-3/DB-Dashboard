...
try:
    ...
    if not df.empty:
        ...
        # 1. 진행 중인 데이터 (남은기간이 '만료됨'이 아닌 것)
        active_df = df[df['남은기간'] != "만료됨"].copy()
        
        # 2. 만료된 데이터 중 2025년 데이터만 추출
        expired_2025_df = df[(df['남은기간'] == "만료됨") & (df['★가공_계약만료일'].str.startswith('2025'))].copy()

        # 각 데이터셋별 중복 제거 (최신순)
        active_df = active_df.sort_values('temp_date', ascending=False).drop_duplicates(['★가공_수요기관', 'contract_group_key', '★가공_업체명'])
        expired_2025_df = expired_2025_df.sort_values('temp_date', ascending=False).drop_duplicates(['★가공_수요기관', 'contract_group_key', '★가공_업체명'])

        # [핵심 로직] 진행 중인 계약이 없는 기관에 대해 2025년 만료 데이터 보완
        agencies_with_active = active_df['★가공_수요기관'].unique()
        fallback_df = expired_2025_df[~expired_2025_df['★가공_수요기관'].isin(agencies_with_active)].copy()
        fallback_df['남은기간'] = "25년 만료(직전계약)"
        ...
        def get_metro_name(agency):
            for metro in METRO_LIST[1:]:
                if str(agency).startswith(metro): return metro
            return "기타"
